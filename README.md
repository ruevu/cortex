...
- **Bulk-write fast path:** `internal/indexer/internal/cbm/sqlite_writer.c` constructs the SQLite file via raw B-tree page writes for full-index runs. Schema-aware after Phase 4 (writes Cortex's `nodes`/`edges` directly). Linear extrapolation: ~3 minutes for a Linux-scale (~180k LOC) repo.
