 ...
 *
 * Workflow:
 *   1. ctx_minhash_compute()  — during AST extraction (per function)
 *   2. ctx_minhash_jaccard()  — pairwise similarity (K=64 agreement)
 *   3. ctx_lsh_*              — locality-sensitive hashing for O(n)
 *                               candidate generation
 */
#ifndef CTX_MINHASH_H
#define CTX_MINHASH_H
 ...
 * Returns false if the body has fewer than CTX_MINHASH_MIN_NODES
...
typedef struct ctx_lsh_index ctx_lsh_index_t;

/* Entry stored in the LSH index. */
typedef struct {
    int64_t node_id;
    const ctx_minhash_t *fingerprint;
