 ...
 *   1. Re-extract calls (ctx_extract_file)
...
static const char *itoa_log(int val) {
    static CTX_TLS char bufs[PC_RING][CTX_SZ_32];
...
static char *extract_local_name_from_json(const char *props_json) {
    ...
    return ctx_strndup(start, end - start);
}
...
static void handle_route_registration(ctx_pipeline_ctx_t *ctx, const CtxCall *call,
    ...
    int64_t route_id = ctx_gbuf_upsert_node(ctx->gbuf, "Route", call->first_string_arg, route_qn,
                                            "", 0, 0, route_props);
    char props[CTX_SZ_512];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"route_registration\"}",
             call->callee_name, call->first_string_arg);
    ctx_gbuf_insert_edge(ctx->gbuf, source_node->id, route_id, "CALLS", props);
    if (call->second_arg_name != NULL && call->second_arg_name[0] != '\0') {
        ctx_resolution_t hres = ctx_registry_resolve(ctx->registry, call->second_arg_name,
                                                     module_qn, imp_keys, imp_vals, imp_count);
        if (hres.qualified_name != NULL && hres.qualified_name[0] != '\0') {
            const ctx_gbuf_node_t *handler = ctx_gbuf_find_by_qn(ctx->gbuf, hres.qualified_name);
            if (handler != NULL) {
                char hprops[CTX_SZ_256];
                snprintf(hprops, sizeof(hprops), "{\"handler\":\"%s\"}", hres.qualified_name);
                ctx_gbuf_insert_edge(ctx->gbuf, handler->id, route_id, "HANDLES", hprops);
            }
        }
    }
}
...
static int64_t create_svc_route_node(ctx_pipeline_ctx_t *ctx, const char *url, ctx_svc_kind_t svc,
...
static void emit_http_async_edge(ctx_pipeline_ctx_t *ctx, const CtxCall *call,
    ...
    bool is_topic = (url_or_topic && url_or_topic[0] != '\0' && svc == CTX_SVC_ASYNC &&
    ...
    if (!is_url && !is_topic) {
        ...
        snprintf(props, sizeof(props),
                 ...
                 res->candidate_count);
        ctx_gbuf_insert_edge(ctx->gbuf, source->id, target->id, "CALLS", props);
        return;
    }
    ...
    ctx_json_escape(esc_callee, sizeof(esc_callee), call->callee_name);
    ctx_json_escape(esc_url, sizeof(esc_url), url_or_topic);
    char props[CTX_SZ_512];
    ...
    ctx_gbuf_insert_edge(ctx->gbuf, source->id, route_id, edge_type, props);
}
...
static void emit_classified_edge(ctx_pipeline_ctx_t *ctx, const CtxCall *call,
    ...
    if (svc == CTX_SVC_ROUTE_REG && call->first_string_arg && call->first_string_arg[0] == '/') {
        handle_route_registration(ctx, call, source, module_qn, imp_keys, imp_vals, imp_count);
        return;
    }
    if (svc == CTX_SVC_HTTP || svc == CTX_SVC_ASYNC) {
        emit_http_async_edge(ctx, call, source, target, res, svc);
        return;
    }
    if (svc == CTX_SVC_CONFIG) {
        char esc_c[CTX_SZ_256];
        char esc_k[CTX_SZ_256];
        ctx_json_escape(esc_c, sizeof(esc_c), call->callee_name);
        ctx_json_escape(esc_k, sizeof(esc_k), call->first_string_arg ? call->first_string_arg : "");
        char props[CTX_SZ_512];
        snprintf(props, sizeof(props), "{\"callee\":\"%s\",\"key\":\"%s\",\"confidence\":%.2f}",
                 esc_c, esc_k, res->confidence);
        ctx_gbuf_insert_edge(ctx->gbuf, source->id, target->id, "CONFIGURES", props);
        return;
    }
    char esc_c2[CTX_SZ_256];
    ctx_json_escape(esc_c2, sizeof(esc_c2), call->callee_name);
    char props[CTX_SZ_512];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"confidence\":%.2f,\"strategy\":\"%s\",\"candidates\":%d}",
...
static const ctx_gbuf_node_t *calls_find_source(ctx_pipeline_ctx_t *ctx, const char *rel,
...
int ctx_pipeline_pass_calls(ctx_pipeline_ctx_t *ctx, const ctx_file_info_t *files, int file_count) {
    ctx_log_info("pass.start", "pass", "calls", "files", itoa_log(file_count));
    ...
    ctx_log_info("pass.done", "pass", "calls", "total", itoa_log(total_calls), "resolved",
    ...
    ctx_pipeline_pass_fastapi_depends(ctx, files, file_count);
...
static int scan_depends_in_sig(ctx_pipeline_ctx_t *ctx, const ctx_regex_t *re, const char *sig,
    ...
    const char *scan = sig;
    while (ctx_regexec(re, scan, PC_REGEX_GRP, match, 0) == 0) {
        int ref_len = match[SKIP_ONE].rm_eo - match[SKIP_ONE].rm_so;
        char func_ref[CTX_SZ_256];
        ...
        func_ref[ref_len] = '\0';
        ctx_resolution_t res = ctx_registry_resolve(ctx->registry, func_ref, module_qn, ik, iv, ic);
        if (res.qualified_name && res.qualified_name[0] != '\0') {
            const ctx_gbuf_node_t *sn = ctx_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
            const ctx_gbuf_node_t *tn = ctx_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
            if (sn && tn && sn->id != tn->id) {
                ctx_gbuf_insert_edge(ctx->gbuf, sn->id, tn->id, "CALLS",
...
void ctx_pipeline_pass_fastapi_depends(ctx_pipeline_ctx_t *ctx, const ctx_file_info_t *files,
                                       int file_count) {
    ctx_regex_t depends_re;
    if (ctx_regcomp(&depends_re, "Depends\\(([A-Za-z_][A-Za-z0-9_.]*)", CTX_REG_EXTENDED) != 0) {
        return;
    }
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        char *module_qn = ctx_pipeline_fqn_module(ctx->project_name, files[i].rel_path);
    ...
    ctx_regfree(&depends_re);
    if (edge_count > 0) {
        ctx_log_info("pass.fastapi_depends", "edges", itoa_log(edge_count));
    }
}
