...
static bool is_dep_section(const char *s) {
    ...
    for (int i = 0; secs[i]; i++) {
        if (ctx_strcasestr(s, secs[i]) != NULL) {
            return true;
        }
    }
    return false;
...
typedef struct {
    int64_t node_id;
    char normalized[CTX_SZ_256];
    char name[CTX_SZ_256];
} config_entry_t;

/* Collect config Variable nodes with ≥2 tokens, each ≥3 chars. */
static int collect_config_entries(const ctx_gbuf_node_t *const *vars, int var_count,
                                  config_entry_t *out, int max_out) {
    int n = 0;
    for (int i = 0; i < var_count && n < max_out; i++) {
        if (!ctx_has_config_extension(vars[i]->file_path)) {
            continue;
        }

        char norm[CTX_SZ_256];
        int tokens = ctx_normalize_config_key(vars[i]->name, norm, sizeof(norm));
        if (tokens < PAIR_LEN) {
            continue;
        }
...
static int collect_code_entries(ctx_gbuf_t *gb, code_entry_t *out, int max_out) {
    int n = 0;
    ...
    for (int li = 0; labels[li] && n < max_out; li++) {
        ...
        for (int i = 0; i < count && n < max_out; i++) {
            if (ctx_has_config_extension(nodes[i]->file_path)) {
                continue;
            }

            char norm[CTX_SZ_256];
            int tokens = ctx_normalize_config_key(nodes[i]->name, norm, sizeof(norm));
...
static int strategy_key_symbols(ctx_gbuf_t *gb) {
    /* Get all Variable nodes */
    const ctx_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (ctx_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    }

    config_entry_t config_entries[CTX_SZ_4K];
    int config_count = collect_config_entries(vars, var_count, config_entries, CTX_SZ_4K);

    if (config_count == 0) {
        return 0;
    }

    code_entry_t code_entries[CTX_SZ_8K];
    int code_count = collect_code_entries(gb, code_entries, CTX_SZ_8K);
    ...
    for (int ci = 0; ci < config_count; ci++) {
        for (int co = 0; co < code_count; co++) {
            ...
            if (confidence > 0.0) {
                ...
                ctx_gbuf_insert_edge(gb, code_entries[co].node_id, config_entries[ci].node_id,
                                     "CONFIGURES", props);
                edge_count++;
            }
        }
...
static int collect_manifest_deps(const ctx_gbuf_node_t *const *vars, int var_count,
...
static double match_dep_to_import(const ctx_gbuf_node_t *target, const char *dep_lower) {
    char target_lower[CTX_SZ_256];
...
static int strategy_dep_imports(ctx_gbuf_t *gb) {
    const ctx_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (ctx_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    ...
    dep_entry_t deps[CTX_SZ_2K];
    int dep_count = collect_manifest_deps(vars, var_count, deps, CTX_SZ_2K);
    ...
    const ctx_gbuf_edge_t **imports = NULL;
    int import_count = 0;
    if (ctx_gbuf_find_edges_by_type(gb, "IMPORTS", &imports, &import_count) != 0) {
        return 0;
    }
    ...
    for (int di = 0; di < dep_count; di++) {
        char dep_lower[CTX_SZ_256];
        lowercase_into(dep_lower, sizeof(dep_lower), deps[di].name);

        for (int ii = 0; ii < import_count; ii++) {
            const ctx_gbuf_node_t *target = ctx_gbuf_find_by_id(gb, imports[ii]->target_id);
            if (!target) {
                continue;
            }

            const ctx_gbuf_node_t *source = ctx_gbuf_find_by_id(gb, imports[ii]->source_id);
            if (!source) {
                continue;
            }
            ...
            if (confidence > 0.0) {
                ...
                ctx_gbuf_insert_edge(gb, source->id, deps[di].node_id, "CONFIGURES", props);
                edge_count++;
            }
        }
    }
...
int ctx_pipeline_pass_configlink(ctx_pipeline_ctx_t *ctx) {
    ctx_gbuf_t *gb = ctx->gbuf;
    /* Early exit: check if any config files exist in the project. */
    bool has_config = false;

    const ctx_gbuf_node_t **vars_check = NULL;
    int var_check_count = 0;
    if (!has_config && ctx_gbuf_find_by_label(gb, "Variable", &vars_check, &var_check_count) == 0) {
        for (int i = 0; i < var_check_count; i++) {
            if (ctx_has_config_extension(vars_check[i]->file_path)) {
                has_config = true;
                break;
            }
        }
    ...
    if (!has_config) {
        const ctx_gbuf_node_t **mods_check = NULL;
        int mod_check_count = 0;
        if (ctx_gbuf_find_by_label(gb, "Module", &mods_check, &mod_check_count) == 0) {
            for (int i = 0; i < mod_check_count; i++) {
                if (ctx_has_config_extension(mods_check[i]->file_path)) {
                    has_config = true;
                    break;
                }
            }
    ...
    if (!has_config) {
        ctx_log_info("configlinker.skip", "reason", "no_config_files");
        return 0;
    }
    ...
    ctx_log_info("configlinker.strategy", "name", "key_symbol", "edges", buf1);

    int dep_edges = strategy_dep_imports(gb);
    snprintf(buf2, sizeof(buf2), "%d", dep_edges);
    ctx_log_info("configlinker.strategy", "name", "dep_import", "edges", buf2);
    ...
    ctx_log_info("configlinker.strategy", "name", "file_ref", "edges", buf3);

    snprintf(buf4, sizeof(buf4), "%d", key_edges + dep_edges + ref_edges);
    ctx_log_info("configlinker.done", "total", buf4);
