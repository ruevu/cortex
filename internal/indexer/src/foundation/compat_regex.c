...
_Static_assert(sizeof(regex_t) <= CTX_SZ_256,
               "ctx_regex_t opaque buffer too small for TRE regex_t");

static int translate_flags_tre(int flags) {
    int tre_flags = 0;
    if (flags & CTX_REG_EXTENDED)
        tre_flags |= REG_EXTENDED;
    if (flags & CTX_REG_ICASE)
        tre_flags |= REG_ICASE;
    if (flags & CTX_REG_NOSUB)
        tre_flags |= REG_NOSUB;
    if (flags & CTX_REG_NEWLINE)
...
int ctx_regexec(const ctx_regex_t *r, const char *str, int nmatch, ctx_regmatch_t *matches,
    ...
    if (nmatch <= 0 || !matches) {
        ...
        return rc == 0 ? CTX_REG_OK : CTX_REG_NOMATCH;
    }
    regmatch_t pmatch[CTX_SZ_32];
    int n = nmatch > CTX_SZ_32 ? CTX_SZ_32 : nmatch;
    int rc = tre_regexec(re, str, (size_t)n, pmatch, eflags);
    if (rc != 0)
        return CTX_REG_NOMATCH;
    for (int i = 0; i < n; i++) {
 ...
 * If this fires, increase ctx_regex_t.opaque size. */
_Static_assert(sizeof(regex_t) <= CTX_SZ_256, "ctx_regex_t opaque buffer too small for regex_t");
