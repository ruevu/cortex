 ...
 */
#ifndef CTX_STR_UTIL_H
#define CTX_STR_UTIL_H
 ...
 *
 * Usage: CTX_SNPRINTF_APPEND(buf, sizeof(buf), off, "fmt %s", arg);
 * Requires: <stdio.h> included by caller. */
#define CTX_SNPRINTF_APPEND(buf, sz, off, ...)                                       \
    do {                                                                             \
        if ((off) >= 0 && (off) < (int)(sz)) {                                       \
            int _ctx_r = snprintf((buf) + (off), (sz) - (size_t)(off), __VA_ARGS__); \
            if (_ctx_r > 0)                                                          \
                (off) += _ctx_r;                                                     \
            if ((off) >= (int)(sz))                                                  \
                (off) = (int)(sz) - 1;                                               \
