...
ctx_query_stats_t g_query_stats = {0};
static atomic_int g_diag_stop = 0;
static ctx_thread_t g_diag_thread;
static bool g_diag_started = false;
static time_t g_start_time = 0;
static char g_diag_path[CTX_SZ_256] = "";
...
static int count_open_fds(void) {
    ...
    return n - PAIR_LEN; /* . and .. */
#else
    return CTX_NOT_FOUND; /* Not available on Windows */
#endif
}
...
bool ctx_diag_start(void) {
    ...
    ctx_safe_getenv("CTX_DIAGNOSTICS", env_buf, sizeof(env_buf), NULL);
...
void ctx_diag_stop(void) {
    ...
    ctx_thread_join(&g_diag_thread);
    g_diag_started = false;

    /* Clean up file */
    ctx_unlink(g_diag_path);
    char tmp_path[sizeof(g_diag_path) + DIAG_PATH_EXTRA];
    snprintf(tmp_path, sizeof(tmp_path), "%s.tmp", g_diag_path);
    ctx_unlink(tmp_path);
}
