 ...
 */
#ifndef CTX_GRAPH_BUFFER_H
#define CTX_GRAPH_BUFFER_H
...
typedef struct ctx_gbuf ctx_gbuf_t;
 ...
 * If id_source is NULL, behaves like ctx_gbuf_new(). */
...
typedef void (*ctx_gbuf_node_visitor_fn)(const ctx_gbuf_node_t *node, void *userdata);
void ctx_gbuf_foreach_node(const ctx_gbuf_t *gb, ctx_gbuf_node_visitor_fn fn, void *userdata);

/* Iterate all edges. */
typedef void (*ctx_gbuf_edge_visitor_fn)(const ctx_gbuf_edge_t *edge, void *userdata);
 ...
 * Vectors are carried through to ctx_write_db during the dump phase. */
