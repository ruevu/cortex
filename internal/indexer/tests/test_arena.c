...
TEST(arena_total) {
    ...
    ctx_arena_alloc(&a, 100);
    ASSERT_GTE(ctx_arena_total(&a), 100); /* at least 100 (may be aligned) */
    ctx_arena_alloc(&a, 200);
    ASSERT_GTE(ctx_arena_total(&a), 300);
    ctx_arena_destroy(&a);
    PASS();
}
...
TEST(arena_multiple_resets) {
    ...
    void *p = ctx_arena_alloc(&a, 60000);
    ASSERT_NOT_NULL(p);
    ctx_arena_alloc(&a, 60000);
    ASSERT_GTE(a.nblocks, 2);
    ctx_arena_reset(&a);
    ASSERT_EQ(a.nblocks, 1);
    ASSERT_EQ(a.used, 0);
...
TEST(arena_strndup_len_exceeds_string) {
    ...
    char *s = ctx_arena_strndup(&a, src, 3);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "abc");
    /* Now test with len matching exactly */
    s = ctx_arena_strndup(&a, src, 2);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "ab");
    ctx_arena_destroy(&a);
    PASS();
...
TEST(arena_many_large_allocs_block_growth) {
    ...
    ASSERT_GTE(ctx_arena_total(&a), 50 * 128);
    ctx_arena_destroy(&a);
    PASS();
}
...
TEST(arena_total_through_reset) {
    ...
    size_t before_reset = ctx_arena_total(&a);
    ...
    ctx_arena_alloc(&a, 50);
    ASSERT_GTE(ctx_arena_total(&a), 50);
    /* But should be less than before_reset (fresh accumulation) */
    ASSERT_LTE(ctx_arena_total(&a), before_reset);
    ctx_arena_destroy(&a);
    PASS();
}
...
TEST(arena_reset_block_size_invariant) {
    ...
    ctx_arena_alloc(&a, 100);
    ctx_arena_alloc(&a, 100); /* triggers grow */
    ASSERT_GTE(a.nblocks, 2);
