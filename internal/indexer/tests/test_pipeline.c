...
static int setup_test_repo(void) {
    snprintf(g_tmpdir, sizeof(g_tmpdir), "/tmp/ctx_test_XXXXXX");
...
TEST(pipeline_create_free) {
    ctx_pipeline_t *p = ctx_pipeline_new("/some/path", NULL, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_STR_EQ(ctx_pipeline_project_name(p), "some-path");
    ctx_pipeline_free(p);
    PASS();
}

TEST(pipeline_null_repo) {
    ctx_pipeline_t *p = ctx_pipeline_new(NULL, NULL, CTX_MODE_FULL);
    ASSERT_NULL(p);
    PASS();
}

TEST(pipeline_free_null) {
    ctx_pipeline_free(NULL); /* should not crash */
    PASS();
}
...
TEST(pipeline_cancel_null) {
    ctx_pipeline_cancel(NULL); /* should not crash */
    PASS();
}

TEST(pipeline_run_null) {
    int rc = ctx_pipeline_run(NULL);
    ASSERT_EQ(rc, -1);
    PASS();
}
...
TEST(store_file_persistence) {
    ...
    ctx_node_t n = {.project = "proj",
                    ...
                    .file_path = "f.go"};
    int64_t id = ctx_store_upsert_node(s1, &n);
    ASSERT_GT(id, 0);
    int cnt1 = ctx_store_count_nodes(s1, "proj");
    ASSERT_EQ(cnt1, 1);
    ctx_store_checkpoint(s1);
    ctx_store_close(s1);

    /* Reopen and verify */
    ctx_store_t *s2 = ctx_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    int cnt2 = ctx_store_count_nodes(s2, "proj");
    ASSERT_EQ(cnt2, 1);
    ctx_store_close(s2);
...
TEST(store_bulk_persistence) {
    ...
    ASSERT_GT(id, 0);
    ASSERT_EQ(ctx_store_commit(s), 0);
    ctx_store_end_bulk(s);
    ctx_store_checkpoint(s);
    ctx_store_close(s);

    /* Reopen and verify data survived */
    ctx_store_t *s2 = ctx_store_open_path(db_path);
    ASSERT_NOT_NULL(s2);
    ASSERT_EQ(ctx_store_count_nodes(s2, "proj"), 1);
    ctx_store_close(s2);
...
TEST(pipeline_structure_nodes) {
    ...
    int node_count = ctx_store_count_nodes(s, project);
    ...
    /* Verify folder nodes */
    ctx_node_t *folders = NULL;
    int folder_count = 0;
    rc = ctx_store_find_nodes_by_label(s, project, "Folder", &folders, &folder_count);
    ASSERT_EQ(rc, CTX_STORE_OK);
    ASSERT_GTE(folder_count, 2); /* pkg, pkg/util */
    ctx_store_free_nodes(folders, folder_count);

    /* Verify file nodes */
    ctx_node_t *file_nodes = NULL;
    int file_count = 0;
    rc = ctx_store_find_nodes_by_label(s, project, "File", &file_nodes, &file_count);
    ASSERT_EQ(rc, CTX_STORE_OK);
    ASSERT_GTE(file_count, 3); /* main.go, service.go, helper.go */
    ctx_store_free_nodes(file_nodes, file_count);

    /* Verify edges exist */
    int edge_count = ctx_store_count_edges(s, project);
...
TEST(pipeline_structure_edges) {
    ...
    int cf_count = ctx_store_count_edges_by_type(s, project, "CONTAINS_FILE");
    /* Check CONTAINS_FOLDER edges */
    int cd_count = ctx_store_count_edges_by_type(s, project, "CONTAINS_FOLDER");
...
TEST(pipeline_project_name_derived) {
    ...
    const char *name = ctx_pipeline_project_name(p);
...
TEST(pipeline_fast_mode) {
    ...
    ctx_pipeline_t *p = ctx_pipeline_new(g_tmpdir, db_path, CTX_MODE_FAST);
    ASSERT_NOT_NULL(p);
...
TEST(pipeline_definitions_function_nodes) {
    ...
    int func_count = 0;
    rc = ctx_store_find_nodes_by_label(s, project, "Function", &funcs, &func_count);
    ASSERT_EQ(rc, CTX_STORE_OK);
    ...
    ctx_store_free_nodes(funcs, func_count);
    ctx_store_close(s);
    ctx_pipeline_free(p);
    teardown_test_repo();
    PASS();
...
TEST(pipeline_definitions_defines_edges) {
    ...
    int defines_count = ctx_store_count_edges_by_type(s, project, "DEFINES");
...
TEST(pipeline_calls_resolution) {
    ...
    int calls_count = ctx_store_count_edges_by_type(s, project, "CALLS");
...
TEST(githistory_is_trackable) {
    ...
    ASSERT_TRUE(ctx_is_trackable_file("main.go"));
    ASSERT_TRUE(ctx_is_trackable_file("src/app.py"));
    ASSERT_TRUE(ctx_is_trackable_file("README.md"));

    /* Non-trackable: skip prefixes */
    ASSERT_FALSE(ctx_is_trackable_file("node_modules/foo/bar.js"));
    ASSERT_FALSE(ctx_is_trackable_file("vendor/lib/dep.go"));
    ASSERT_FALSE(ctx_is_trackable_file(".git/config"));
    ASSERT_FALSE(ctx_is_trackable_file("__pycache__/mod.pyc"));

    /* Non-trackable: lock files */
    ASSERT_FALSE(ctx_is_trackable_file("package-lock.json"));
    ASSERT_FALSE(ctx_is_trackable_file("go.sum"));

    /* Non-trackable: binary/minified extensions */
    ASSERT_FALSE(ctx_is_trackable_file("image.png"));
    ASSERT_FALSE(ctx_is_trackable_file("src/style.min.css"));
...
TEST(githistory_compute_coupling) {
    ...
    ctx_commit_files_t commits[] = {
        {files_0, 3}, {files_1, 2}, {files_2, 2}, {files_3, 2}, {files_4, 2},
    };

    ctx_change_coupling_t results[100];
    int n = ctx_compute_change_coupling(commits, 5, results, 100);
...
TEST(githistory_skip_large_commits) {
    ...
    ctx_commit_files_t commits[] = {{files, 25}};

    ctx_change_coupling_t results[100];
    int n = ctx_compute_change_coupling(commits, 1, results, 100);
    ASSERT_EQ(n, 0);
...
TEST(githistory_limits_to_max) {
    ...
    ctx_commit_files_t *commits = calloc(ncommits, sizeof(ctx_commit_files_t));
    ...
    ctx_change_coupling_t results[100];
    int n = ctx_compute_change_coupling(commits, ncommits, results, 100);
    ASSERT_TRUE(n <= 100);
...
TEST(testdetect_is_test_file) {
    ...
    ASSERT_TRUE(ctx_is_test_path("foo_test.go"));
    ASSERT_TRUE(ctx_is_test_path("test_handler.py"));
    ASSERT_TRUE(ctx_is_test_path("handler.test.js"));
    ASSERT_TRUE(ctx_is_test_path("handler.spec.ts"));
    ASSERT_TRUE(ctx_is_test_path("Component.test.tsx"));
    ASSERT_TRUE(ctx_is_test_path("OrderTest.java"));
    ASSERT_TRUE(ctx_is_test_path("handler_test.rs"));
    ASSERT_TRUE(ctx_is_test_path("handler_test.cpp"));
    ASSERT_TRUE(ctx_is_test_path("OrderTest.cs"));
    ASSERT_TRUE(ctx_is_test_path("OrderTest.php"));
    ASSERT_TRUE(ctx_is_test_path("OrderSpec.scala"));
    ASSERT_TRUE(ctx_is_test_path("OrderTest.kt"));
    ASSERT_TRUE(ctx_is_test_path("handler_test.lua"));

    /* Non-test file patterns */
    ASSERT_FALSE(ctx_is_test_path("foo.go"));
    ASSERT_FALSE(ctx_is_test_path("handler.py"));
    ASSERT_FALSE(ctx_is_test_path("handler.js"));
    ASSERT_FALSE(ctx_is_test_path("handler.ts"));
    ASSERT_FALSE(ctx_is_test_path("Component.tsx"));
    ASSERT_FALSE(ctx_is_test_path("Order.java"));
    ASSERT_FALSE(ctx_is_test_path("handler.rs"));
    ASSERT_FALSE(ctx_is_test_path("handler.cpp"));
    ASSERT_FALSE(ctx_is_test_path("Order.cs"));
    ASSERT_FALSE(ctx_is_test_path("Order.php"));
    ASSERT_FALSE(ctx_is_test_path("Order.scala"));
    ASSERT_FALSE(ctx_is_test_path("Order.kt"));
    ASSERT_FALSE(ctx_is_test_path("handler.lua"));
...
TEST(testdetect_is_test_function) {
    ...
    ASSERT_TRUE(ctx_is_test_func_name("TestCreate"));  /* Go */
    ASSERT_TRUE(ctx_is_test_func_name("test_create")); /* Python/Rust/Lua */
    ASSERT_TRUE(ctx_is_test_func_name("test"));        /* JS/TS */
    ASSERT_TRUE(ctx_is_test_func_name("describe"));    /* JS/TS */
    ASSERT_TRUE(ctx_is_test_func_name("it"));          /* JS/TS */
    ASSERT_TRUE(ctx_is_test_func_name("testCreate"));  /* Java/PHP/Scala/Kotlin */
    ASSERT_TRUE(ctx_is_test_func_name("TestCreate"));  /* C++/C# */

    /* Non-test function patterns */
    ASSERT_FALSE(ctx_is_test_func_name("create"));
    ASSERT_FALSE(ctx_is_test_func_name("handleRequest"));
    ASSERT_FALSE(ctx_is_test_func_name("process"));
...
TEST(implements_creates_override) {
    ...
    ctx_gbuf_t *gb = ctx_gbuf_new("test-proj", "/tmp/test");
    ASSERT_NOT_NULL(gb);

    /* Interface "Reader" with methods "Read" and "Close" */
    int64_t iface_id =
        ctx_gbuf_upsert_node(gb, "Interface", "Reader", "pkg.Reader", "pkg/reader.go", 1, 5, "{}");
    ASSERT_GT(iface_id, 0);

    int64_t read_method_id =
        ctx_gbuf_upsert_node(gb, "Method", "Read", "pkg.Reader.Read", "pkg/reader.go", 2, 2, "{}");
    int64_t close_method_id = ctx_gbuf_upsert_node(gb, "Method", "Close", "pkg.Reader.Close",
                                                   "pkg/reader.go", 3, 3, "{}");
    ASSERT_GT(read_method_id, 0);
    ASSERT_GT(close_method_id, 0);

    /* DEFINES_METHOD edges */
    ctx_gbuf_insert_edge(gb, iface_id, read_method_id, "DEFINES_METHOD", "{}");
    ctx_gbuf_insert_edge(gb, iface_id, close_method_id, "DEFINES_METHOD", "{}");

    /* Class "FileReader" with matching methods */
    int64_t class_id = ctx_gbuf_upsert_node(gb, "Class", "FileReader", "pkg.FileReader",
                                            "pkg/filereader.go", 1, 10, "{}");
    ASSERT_GT(class_id, 0);

    int64_t fr_read_id =
        ctx_gbuf_upsert_node(gb, "Method", "Read", "pkg.FileReader.Read", "pkg/filereader.go", 2, 4,
                             "{\"receiver\":\"(f *FileReader)\"}");
    int64_t fr_close_id =
        ctx_gbuf_upsert_node(gb, "Method", "Close", "pkg.FileReader.Close", "pkg/filereader.go", 5,
    ...
    int edges_created = ctx_pipeline_implements_go(&ctx);
    ASSERT_GT(edges_created, 0);

    /* Verify IMPLEMENTS edge: FileReader → Reader */
    const ctx_gbuf_edge_t **impl_edges = NULL;
    int impl_count = 0;
    int rc =
        ctx_gbuf_find_edges_by_source_type(gb, class_id, "IMPLEMENTS", &impl_edges, &impl_count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(impl_count, 1);
    ASSERT_EQ(impl_edges[0]->target_id, iface_id);

    /* Verify OVERRIDE edges: FileReader.Read → Reader.Read */
    const ctx_gbuf_edge_t **read_overrides = NULL;
    int read_override_count = 0;
    rc = ctx_gbuf_find_edges_by_source_type(gb, fr_read_id, "OVERRIDE", &read_overrides,
                                            &read_override_count);
    ASSERT_EQ(rc, 0);
    ...
    const ctx_gbuf_edge_t **close_overrides = NULL;
    int close_override_count = 0;
    rc = ctx_gbuf_find_edges_by_source_type(gb, fr_close_id, "OVERRIDE", &close_overrides,
...
TEST(implements_no_match) {
    ...
    int64_t iface_id = ctx_gbuf_upsert_node(gb, "Interface", "ReadWriter", "pkg.ReadWriter",
                                            "pkg/rw.go", 1, 5, "{}");
    int64_t read_id =
        ctx_gbuf_upsert_node(gb, "Method", "Read", "pkg.ReadWriter.Read", "pkg/rw.go", 2, 2, "{}");
    int64_t write_id = ctx_gbuf_upsert_node(gb, "Method", "Write", "pkg.ReadWriter.Write",
                                            "pkg/rw.go", 3, 3, "{}");
    ctx_gbuf_insert_edge(gb, iface_id, read_id, "DEFINES_METHOD", "{}");
    ctx_gbuf_insert_edge(gb, iface_id, write_id, "DEFINES_METHOD", "{}");

    /* Struct "OnlyReader" with only "Read" (missing "Write") */
    int64_t class_id = ctx_gbuf_upsert_node(gb, "Class", "OnlyReader", "pkg.OnlyReader",
                                            "pkg/onlyreader.go", 1, 10, "{}");
    int64_t or_read_id =
        ctx_gbuf_upsert_node(gb, "Method", "Read", "pkg.OnlyReader.Read", "pkg/onlyreader.go", 2, 4,
    ...
    const ctx_gbuf_edge_t **override_edges = NULL;
    int override_count = 0;
    ctx_gbuf_find_edges_by_source_type(gb, or_read_id, "OVERRIDE", &override_edges,
                                       &override_count);
...
static int setup_usages_repo(const char *filename, const char *content, const char *extra_file,
    ...
    snprintf(g_usages_tmpdir, sizeof(g_usages_tmpdir), "/tmp/ctx_usage_XXXXXX");
    if (!ctx_mkdtemp(g_usages_tmpdir))
        return -1;
...
TEST(usages_creates_edges) {
    ...
    ctx_pipeline_t *p = ctx_pipeline_new(g_usages_tmpdir, db_path, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    int rc = ctx_pipeline_run(p);
    ASSERT_EQ(rc, 0);
    ...
    int edge_count = 0;
    rc = ctx_store_find_edges_by_type(s, project, "USAGE", &edges, &edge_count);

    bool found_usage = false;
    for (int i = 0; i < edge_count; i++) {
        ctx_node_t src = {0}, tgt = {0};
        if (ctx_store_find_node_by_id(s, edges[i].source_id, &src) == CTX_STORE_OK &&
            ctx_store_find_node_by_id(s, edges[i].target_id, &tgt) == CTX_STORE_OK) {
            if (strcmp(src.name, "Register") == 0 && strcmp(tgt.name, "Process") == 0) {
                found_usage = true;
            }
        }
        ctx_node_free_fields(&src);
        ctx_node_free_fields(&tgt);
    }
    if (edges)
...
TEST(usages_no_duplicate_calls) {
    ...
    ctx_edge_t *call_edges = NULL;
    int call_count = 0;
    ctx_store_find_edges_by_type(s, project, "CALLS", &call_edges, &call_count);

    bool found_call = false;
    for (int i = 0; i < call_count; i++) {
        ctx_node_t src = {0}, tgt = {0};
        if (ctx_store_find_node_by_id(s, call_edges[i].source_id, &src) == CTX_STORE_OK &&
            ctx_store_find_node_by_id(s, call_edges[i].target_id, &tgt) == CTX_STORE_OK) {
    ...
    if (call_edges)
        ctx_store_free_edges(call_edges, call_count);
    ASSERT_TRUE(found_call);

    /* Should NOT have USAGE edge from Main to Helper */
    ctx_edge_t *usage_edges = NULL;
    int usage_count = 0;
    ctx_store_find_edges_by_type(s, project, "USAGE", &usage_edges, &usage_count);

    for (int i = 0; i < usage_count; i++) {
        ctx_node_t src = {0}, tgt = {0};
        if (ctx_store_find_node_by_id(s, usage_edges[i].source_id, &src) == CTX_STORE_OK &&
            ctx_store_find_node_by_id(s, usage_edges[i].target_id, &tgt) == CTX_STORE_OK) {
    ...
    if (usage_edges)
        ctx_store_free_edges(usage_edges, usage_count);
...
static int setup_lang_repo(const char **filenames, const char **contents, int count) {
    snprintf(g_lang_tmpdir, sizeof(g_lang_tmpdir), "/tmp/ctx_lang_XXXXXX");
    if (!ctx_mkdtemp(g_lang_tmpdir))
        return -1;
...
TEST(pipeline_python_project) {
    ...
    ctx_pipeline_t *p = ctx_pipeline_new(g_lang_tmpdir, db, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(ctx_pipeline_run(p), 0);

    ctx_store_t *s = ctx_store_open_path(db);
    ASSERT_NOT_NULL(s);
    const char *proj = ctx_pipeline_project_name(p);

    ctx_node_t *funcs = NULL;
    int fc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Function", &funcs, &fc);
    ASSERT_GTE(fc, 3); /* greet, process, fetch_data */
    ctx_store_free_nodes(funcs, fc);

    ctx_node_t *cls = NULL;
    int cc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Class", &cls, &cc);
    ASSERT_GTE(cc, 1); /* DataProcessor */
    ctx_store_free_nodes(cls, cc);

    ctx_node_t *methods = NULL;
    int mc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Method", &methods, &mc);
    ASSERT_GTE(mc, 1); /* transform */
    ctx_store_free_nodes(methods, mc);
...
TEST(pipeline_go_cross_package_call) {
    ...
    ctx_node_t *targets = NULL;
    int tc = 0;
    ctx_store_find_nodes_by_name(s, proj, "ProcessOrder", &targets, &tc);
    ASSERT_GT(tc, 0);

    /* Verify run() exists */
    ctx_node_t *callers = NULL;
    int clc = 0;
    ctx_store_find_nodes_by_name(s, proj, "run", &callers, &clc);
    ASSERT_GT(clc, 0);

    /* Check CALLS edge from run to ProcessOrder */
    ctx_edge_t *edges = NULL;
    int ec = 0;
    ctx_store_find_edges_by_source_type(s, callers[0].id, "CALLS", &edges, &ec);
    bool found = false;
    for (int i = 0; i < ec; i++) {
    ...
    if (edges)
        ctx_store_free_edges(edges, ec);
    ctx_store_free_nodes(targets, tc);
    ctx_store_free_nodes(callers, clc);
    ctx_store_close(s);
    ctx_pipeline_free(p);
    teardown_lang_repo();
    PASS();
...
TEST(pipeline_python_cross_module_call) {
    ...
    ctx_node_t *targets = NULL;
    int tc = 0;
    ctx_store_find_nodes_by_name(s, proj, "fetch_data", &targets, &tc);
    ASSERT_GT(tc, 0);

    ctx_node_t *callers = NULL;
    int clc = 0;
    ctx_store_find_nodes_by_name(s, proj, "process", &callers, &clc);
    ASSERT_GT(clc, 0);
...
TEST(pipeline_go_type_classification) {
    ...
    ctx_node_t *ifaces = NULL;
    int ic = 0;
    ctx_store_find_nodes_by_label(s, proj, "Interface", &ifaces, &ic);
    ASSERT_EQ(ic, 2);
    ctx_store_free_nodes(ifaces, ic);
    ...
    ctx_node_t *types = NULL;
    int tc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Type", &types, &tc);
    ASSERT_EQ(tc, 1);
    ASSERT_STR_EQ(types[0].name, "ID");
    ctx_store_free_nodes(types, tc);
...
TEST(pipeline_kotlin_project) {
    ...
    int edge_count = ctx_store_count_edges(s, proj);
...
TEST(pipeline_lua_anonymous_functions) {
    ...
    for (int e = 0; e < 3; e++) {
        ctx_node_t *found = NULL;
        int fnc = 0;
        ctx_store_find_nodes_by_name(s, proj, expected[e], &found, &fnc);
        ASSERT_GT(fnc, 0);
        ctx_store_free_nodes(found, fnc);
    }
...
TEST(pipeline_csharp_modern) {
    ...
    ctx_node_t *modules = NULL;
    int modc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Module", &modules, &modc);
    ASSERT_GTE(modc, 2);
    ctx_store_free_nodes(modules, modc);
...
TEST(pipeline_bom_stripping) {
    ...
    snprintf(g_lang_tmpdir, sizeof(g_lang_tmpdir), "/tmp/ctx_bom_XXXXXX");
    if (!ctx_mkdtemp(g_lang_tmpdir))
        SKIP("tmpdir");
    ...
    ctx_node_t *found = NULL;
    int fc = 0;
    ctx_store_find_nodes_by_name(s, proj, "BOMFunc", &found, &fc);
    ASSERT_GT(fc, 0); /* BOMFunc should be found despite BOM */
    ctx_store_free_nodes(found, fc);
...
TEST(pipeline_form_call_resolution) {
    ...
    ctx_edge_t *edges = NULL;
    int ec = 0;
    ctx_store_find_edges_by_type(s, proj, "CALLS", &edges, &ec);
    bool found = false;
    for (int i = 0; i < ec; i++) {
        ctx_node_t tgt = {0};
        if (ctx_store_find_node_by_id(s, edges[i].target_id, &tgt) == CTX_STORE_OK &&
...
TEST(pipeline_python_type_inference) {
    ...
    ctx_node_t *methods = NULL;
    int mc = 0;
    ctx_store_find_nodes_by_name(s, proj, "transform", &methods, &mc);
    ASSERT_GT(mc, 0);
...
TEST(pipeline_docstring_go_function) {
    ...
    int nc = 0;
    ctx_store_find_nodes_by_name(s, proj, "Compute", &nodes, &nc);
    ASSERT_GT(nc, 0);
...
TEST(pipeline_docstring_go_class) {
    ...
    ctx_node_t *nodes = NULL;
    int nc = 0;
    ctx_store_find_nodes_by_name(s, proj, "MyStruct", &nodes, &nc);
    ASSERT_GT(nc, 0);
...
TEST(project_name_from_path) {
    ...
    for (int i = 0; i < 4; i++) {
        char *got = ctx_project_name_from_path(cases[i].path);
        ASSERT_NOT_NULL(got);
...
TEST(project_name_uniqueness) {
    ...
    char *a = ctx_project_name_from_path("/tmp/bench/zig/lib/std");
    char *b = ctx_project_name_from_path("/tmp/bench/erlang/lib/stdlib/src");
    ASSERT_NOT_NULL(a);
    ASSERT_NOT_NULL(b);
...
TEST(gitdiff_parse_range_with_count) {
    int start, count;
    ctx_parse_range("10,5", &start, &count);
    ASSERT_EQ(start, 10);
    ASSERT_EQ(count, 5);
    PASS();
}
...
TEST(gitdiff_parse_range_no_count) {
    int start, count;
    ctx_parse_range("10", &start, &count);
    ASSERT_EQ(start, 10);
    ASSERT_EQ(count, 1);
    PASS();
}
...
TEST(gitdiff_parse_range_zero_count) {
    int start, count;
    ctx_parse_range("1,0", &start, &count);
    ASSERT_EQ(start, 1);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(gitdiff_parse_range_large) {
    int start, count;
    ctx_parse_range("52,2", &start, &count);
    ASSERT_EQ(start, 52);
    ASSERT_EQ(count, 2);
    PASS();
}
...
TEST(gitdiff_parse_name_status) {
    ...
    ctx_changed_file_t files[16];
    int n = ctx_parse_name_status(input, files, 16);
...
TEST(gitdiff_parse_hunks) {
    ...
    ctx_changed_hunk_t hunks[16];
    int n = ctx_parse_hunks(input, hunks, 16);
...
TEST(gitdiff_parse_hunks_no_newline_marker) {
    ...
    ctx_changed_hunk_t hunks[4];
    int n = ctx_parse_hunks(input, hunks, 4);
...
TEST(configures_is_env_var_name) {
    ASSERT(ctx_is_env_var_name("DATABASE_URL"));
    ASSERT(ctx_is_env_var_name("API_KEY"));
    ASSERT(ctx_is_env_var_name("PORT"));
    ASSERT(!ctx_is_env_var_name("A"));      /* too short */
    ASSERT(!ctx_is_env_var_name("port"));   /* lowercase */
    ASSERT(!ctx_is_env_var_name("apiKey")); /* camelCase */
    ASSERT(ctx_is_env_var_name("DB_2"));    /* with digit */
    ASSERT(!ctx_is_env_var_name("__"));     /* no uppercase */
    ASSERT(!ctx_is_env_var_name(""));       /* empty */
    PASS();
}
...
TEST(configures_normalize_config_key) {
    ...
    tokens = ctx_normalize_config_key("max_connections", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "max_connections");
    ASSERT_EQ(tokens, 2);

    tokens = ctx_normalize_config_key("maxConnections", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "max_connections");
    ASSERT_EQ(tokens, 2);

    tokens = ctx_normalize_config_key("DATABASE_HOST", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "database_host");
    ASSERT_EQ(tokens, 2);

    tokens = ctx_normalize_config_key("database.host", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "database_host");
    ASSERT_EQ(tokens, 2);

    tokens = ctx_normalize_config_key("port", norm, sizeof(norm));
    ASSERT_STR_EQ(norm, "port");
    ASSERT_EQ(tokens, 1);

    tokens = ctx_normalize_config_key("maxRetryCount", norm, sizeof(norm));
...
TEST(configures_has_config_extension) {
    ASSERT(ctx_has_config_extension("config.toml"));
    ASSERT(ctx_has_config_extension("settings.yaml"));
    ASSERT(ctx_has_config_extension("config.yml"));
    ASSERT(ctx_has_config_extension(".env"));
    ASSERT(ctx_has_config_extension("config.ini"));
    ASSERT(ctx_has_config_extension("data.json"));
    ASSERT(ctx_has_config_extension("pom.xml"));
    ASSERT(!ctx_has_config_extension("main.go"));
    ASSERT(!ctx_has_config_extension("app.py"));
    ASSERT(!ctx_has_config_extension("data.csv"));
    PASS();
}
...
TEST(configures_env_var_in_config) {
    ...
    ctx_edge_t *edges = NULL;
    int ec = 0;
    ctx_store_find_edges_by_type(s, proj, "CONFIGURES", &edges, &ec);
...
TEST(configures_full_pipeline_integration) {
    ...
    ctx_node_t *classes = NULL;
    int cc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Class", &classes, &cc);
    ASSERT_GT(cc, 0);
    if (classes)
        ctx_store_free_nodes(classes, cc);

    /* Should have Variable nodes from config files */
    ctx_node_t *vars = NULL;
    int vc = 0;
    ctx_store_find_nodes_by_label(s, proj, "Variable", &vars, &vc);
    ASSERT_GT(vc, 0);
    if (vars)
        ctx_store_free_nodes(vars, vc);
...
TEST(enrichment_split_camel_case) {
    ...
    n = ctx_split_camel_case("GetMapping", parts, 8);
    ASSERT_EQ(n, 2);
    ...
    n = ctx_split_camel_case("getMessage", parts, 8);
    ASSERT_EQ(n, 2);
    ASSERT_STR_EQ(parts[0], "get");
    ...
    n = ctx_split_camel_case("cache", parts, 8);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(parts[0], "cache");
    for (int i = 0; i < n; i++)
        free(parts[i]);

    n = ctx_split_camel_case("HTMLParser", parts, 8);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(parts[0], "HTMLParser");
    for (int i = 0; i < n; i++)
        free(parts[i]);

    n = ctx_split_camel_case("", parts, 8);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(enrichment_tokenize_decorator) {
    ...
    n = ctx_tokenize_decorator("@Override", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "override");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = ctx_tokenize_decorator("@Deprecated", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "deprecated");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = ctx_tokenize_decorator("@Test", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "test");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = ctx_tokenize_decorator("@login_required", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = ctx_tokenize_decorator("@cache", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "cache");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = ctx_tokenize_decorator("@pytest.fixture", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = ctx_tokenize_decorator("@GetMapping(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 1);
    ...
    n = ctx_tokenize_decorator("@PostMapping(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = ctx_tokenize_decorator("@Transactional", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "transactional");
    for (int i = 0; i < n; i++)
        free(tokens[i]);

    n = ctx_tokenize_decorator("@MessageMapping(\"/chat\")", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    /* #[derive(Debug)] */
    n = ctx_tokenize_decorator("#[derive(Debug)]", tokens, 16);
    ASSERT_EQ(n, 1);
    ...
    n = ctx_tokenize_decorator("@app.get(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 0);

    /* "router" is stopword, "post" passes */
    n = ctx_tokenize_decorator("@router.post(\"/api\")", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "post");
    for (int i = 0; i < n; i++)
        free(tokens[i]);
    ...
    n = ctx_tokenize_decorator("@x", tokens, 16);
    ASSERT_EQ(n, 0);

    /* Empty */
    n = ctx_tokenize_decorator("", tokens, 16);
    ASSERT_EQ(n, 0);

    n = ctx_tokenize_decorator("@click.command", tokens, 16);
    ASSERT_EQ(n, 2);
    ...
    n = ctx_tokenize_decorator("@celery.task", tokens, 16);
    ASSERT_EQ(n, 2);
...
TEST(compile_commands_split_command) {
    ...
    n = ctx_split_command("gcc -c main.c", args, 16);
    ASSERT_EQ(n, 3);
    ...
    n = ctx_split_command("gcc -DFOO=\"bar baz\" -c main.c", args, 16);
    ASSERT_EQ(n, 4);
    for (int i = 0; i < n; i++)
        free(args[i]);

    n = ctx_split_command("g++ -I/usr/include -std=c++17 -o out -c in.cpp", args, 16);
    ASSERT_EQ(n, 7);
    for (int i = 0; i < n; i++)
        free(args[i]);
    PASS();
...
TEST(compile_commands_parse_json) {
    ...
    char **paths = NULL;
    ctx_compile_flags_t **flags = NULL;
    int n = ctx_parse_compile_commands(json, "/home/user/project", &paths, &flags);
    ...
    for (int i = 0; i < n; i++) {
        free(paths[i]);
        ctx_compile_flags_free(flags[i]);
    }
    free(paths);
    free(flags);
    PASS();
...
TEST(compile_commands_parse_empty) {
    char **paths = NULL;
    ctx_compile_flags_t **flags = NULL;
    int n = ctx_parse_compile_commands("[]", "/repo", &paths, &flags);
    ASSERT_EQ(n, 0);
    free(paths);
    free(flags);
    PASS();
...
TEST(compile_commands_parse_invalid) {
    char **paths = NULL;
    ctx_compile_flags_t **flags = NULL;
    int n = ctx_parse_compile_commands("not json", "/repo", &paths, &flags);
    ASSERT(n < 0);
    PASS();
}
...
TEST(infra_is_compose_file) {
    ...
    ASSERT(ctx_is_compose_file("docker-compose.yml"));
    ASSERT(ctx_is_compose_file("docker-compose.yaml"));
    ASSERT(ctx_is_compose_file("docker-compose.prod.yml"));
    ASSERT(ctx_is_compose_file("compose.yml"));
    ASSERT(ctx_is_compose_file("compose.yaml"));
    ASSERT(!ctx_is_compose_file("mycompose.yml"));
    ASSERT(!ctx_is_compose_file("docker-compose.txt"));
    ASSERT(!ctx_is_compose_file("Dockerfile"));
    PASS();
}

TEST(infra_is_cloudbuild_file) {
    /* Port of TestIsCloudbuildFile (5 cases) */
    ASSERT(ctx_is_cloudbuild_file("cloudbuild.yaml"));
    ASSERT(ctx_is_cloudbuild_file("cloudbuild.yml"));
    ASSERT(ctx_is_cloudbuild_file("cloudbuild-prod.yaml"));
    ASSERT(ctx_is_cloudbuild_file("Cloudbuild.yml"));
    ASSERT(!ctx_is_cloudbuild_file("build.yaml"));
    PASS();
}

TEST(infra_is_shell_script) {
    /* Port of TestIsShellScript (5 cases) */
    ASSERT(ctx_is_shell_script("run.sh", ".sh"));
    ASSERT(ctx_is_shell_script("deploy.bash", ".bash"));
    ASSERT(ctx_is_shell_script("init.zsh", ".zsh"));
    ASSERT(!ctx_is_shell_script("main.py", ".py"));
    ASSERT(!ctx_is_shell_script("Dockerfile", ""));
    PASS();
}

TEST(infra_is_dockerfile) {
    ASSERT(ctx_is_dockerfile("Dockerfile"));
    ASSERT(ctx_is_dockerfile("dockerfile"));
    ASSERT(ctx_is_dockerfile("Dockerfile.prod"));
    ASSERT(ctx_is_dockerfile("app.dockerfile"));
    ASSERT(!ctx_is_dockerfile("docker-compose.yml"));
    ASSERT(!ctx_is_dockerfile("main.go"));
    PASS();
}

TEST(infra_is_kustomize_file) {
    ASSERT(ctx_is_kustomize_file("kustomization.yaml"));
    ASSERT(ctx_is_kustomize_file("kustomization.yml"));
    ASSERT(ctx_is_kustomize_file("KUSTOMIZATION.YAML")); /* case-insensitive */
    ASSERT(!ctx_is_kustomize_file("deployment.yaml"));
    ASSERT(!ctx_is_kustomize_file("kustomize.yaml"));
    ASSERT(!ctx_is_kustomize_file(NULL));
    PASS();
}
...
TEST(infra_is_k8s_manifest) {
    ...
    ASSERT(ctx_is_k8s_manifest("deployment.yaml", deploy));
    ASSERT(!ctx_is_k8s_manifest("deployment.yaml", plain));
    /* kustomize file should return false even if it has apiVersion */
    ASSERT(!ctx_is_k8s_manifest("kustomization.yaml", kust));
    ASSERT(!ctx_is_k8s_manifest(NULL, deploy));
    ASSERT(!ctx_is_k8s_manifest("deployment.yaml", NULL));
    PASS();
}

TEST(infra_is_env_file) {
    ASSERT(ctx_is_env_file(".env"));
    ASSERT(ctx_is_env_file(".env.local"));
    ASSERT(ctx_is_env_file("prod.env"));
    ASSERT(!ctx_is_env_file("main.go"));
    ASSERT(!ctx_is_env_file("env.txt"));
    PASS();
}
...
TEST(infra_clean_json_brackets) {
    ...
    ctx_clean_json_brackets("[\"./server\"]", out, sizeof(out));
    ASSERT_STR_EQ(out, "./server");

    ctx_clean_json_brackets("[\"python\", \"main.py\"]", out, sizeof(out));
    ASSERT_STR_EQ(out, "python main.py");

    ctx_clean_json_brackets("./server", out, sizeof(out));
    ASSERT_STR_EQ(out, "./server");

    ctx_clean_json_brackets("[\"./app\", \"--flag\", \"value\"]", out, sizeof(out));
...
TEST(infra_secret_detection) {
    /* Key-based detection */
    ASSERT(ctx_is_secret_binding("JWT_SECRET", "anything"));
    ASSERT(ctx_is_secret_binding("API_KEY", "anything"));
    ASSERT(ctx_is_secret_binding("my_password", "anything"));
    ASSERT(ctx_is_secret_binding("AUTH_TOKEN", "anything"));
    ASSERT(!ctx_is_secret_binding("DATABASE_URL", "https://db.example.com"));

    /* Value-based detection */
    ASSERT(ctx_is_secret_value("sk-1234567890abcdef12345"));
    ASSERT(ctx_is_secret_value("-----BEGIN RSA PRIVATE KEY-----"));
    ASSERT(!ctx_is_secret_value("https://db.example.com"));
    ASSERT(!ctx_is_secret_value("hello world"));
    ASSERT(!ctx_is_secret_value("8080"));

    /* isSecretBinding checks both */
    ASSERT(ctx_is_secret_binding("ANYTHING", "sk-1234567890abcdef12345"));
    ASSERT(!ctx_is_secret_binding("PORT", "8080"));
...
static const char *find_env_var(const ctx_env_kv_t *vars, int count, const char *key) {
    for (int i = 0; i < count; i++) {
...
TEST(infra_parse_dockerfile_multistage) {
    ...
    ctx_dockerfile_result_t r;
    ASSERT_EQ(ctx_parse_dockerfile_source(src, &r), 0);
...
TEST(infra_parse_dockerfile_empty) {
    ...
    ctx_dockerfile_result_t r;
    ASSERT_EQ(ctx_parse_dockerfile_source("# just a comment\n", &r), -1);
    PASS();
}
...
TEST(infra_parse_dotenv) {
    ...
    ctx_dotenv_result_t r;
    ASSERT_EQ(ctx_parse_dotenv_source(src, &r), 0);
...
TEST(infra_parse_shell) {
    ...
    ctx_shell_result_t r;
    ASSERT_EQ(ctx_parse_shell_source(src, &r), 0);
...
TEST(infra_parse_shell_truly_empty) {
    ...
    ctx_shell_result_t r;
    ASSERT_EQ(ctx_parse_shell_source("# no shebang, just comments\n", &r), -1);
    PASS();
}
...
TEST(infra_parse_terraform_full) {
    ...
    ctx_terraform_result_t r;
    ASSERT_EQ(ctx_parse_terraform_source(src, &r), 0);
...
TEST(infra_parse_terraform_empty) {
    ...
    ctx_terraform_result_t r;
    ASSERT_EQ(ctx_parse_terraform_source("# just comments\n", &r), -1);
    PASS();
}
...
TEST(registry_resolve_single_candidate) {
    ...
    ctx_registry_add(reg, "CreateOrder", "svcA.handlers.CreateOrder", "Function");
    ctx_registry_add(reg, "ValidateOrder", "svcB.validators.ValidateOrder", "Function");

    /* Normal resolve unique name */
    ctx_resolution_t r = ctx_registry_resolve(reg, "CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_STR_EQ(r.qualified_name, "svcA.handlers.CreateOrder");

    /* Fuzzy resolve with unknown prefix */
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknownPkg.CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_fuzzy_nonexistent) {
    ...
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "NonExistent", "svcC.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(registry_fuzzy_multiple_best_by_distance) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Process", "svcA.handlers.Process", "Function");
    ctx_registry_add(reg, "Process", "svcB.handlers.Process", "Function");

    /* Caller in svcA → prefer svcA */
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknown.Process", "svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_STR_EQ(fr.result.qualified_name, "svcA.handlers.Process");

    /* Caller in svcB → prefer svcB */
    fr = ctx_registry_fuzzy_resolve(reg, "unknown.Process", "svcB.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_fuzzy_simple_name_extraction) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "DoWork", "myproject.utils.DoWork", "Function");

    /* Deeply qualified name → extract "DoWork" */
    ctx_fuzzy_result_t fr = ctx_registry_fuzzy_resolve(reg, "some.deep.module.DoWork",
...
TEST(registry_fuzzy_empty) {
    ...
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "SomeFunc", "myproject.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(registry_exists) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Foo", "pkg.module.Foo", "Function");
    ctx_registry_add(reg, "Bar", "pkg.module.Bar", "Method");

    ASSERT_TRUE(ctx_registry_exists(reg, "pkg.module.Foo"));
    ASSERT_TRUE(ctx_registry_exists(reg, "pkg.module.Bar"));
    ASSERT_FALSE(ctx_registry_exists(reg, "pkg.module.Missing"));
    ASSERT_FALSE(ctx_registry_exists(reg, ""));
...
TEST(registry_confidence_import_map) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Foo", "proj.other.Foo", "Function");

    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    ctx_resolution_t r = ctx_registry_resolve(reg, "other.Foo", "proj.pkg", keys, vals, 1);
...
TEST(registry_confidence_import_map_suffix) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Foo", "proj.other.sub.Foo", "Function");
...
TEST(registry_confidence_same_module) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Foo", "proj.pkg.Foo", "Function");

    ctx_resolution_t r = ctx_registry_resolve(reg, "Foo", "proj.pkg", NULL, NULL, 0);
...
TEST(registry_confidence_unique_name) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Bar", "proj.pkg.Bar", "Function");

    ctx_resolution_t r = ctx_registry_resolve(reg, "Bar", "proj.unrelated", NULL, NULL, 0);
...
TEST(registry_confidence_suffix_match) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Process", "proj.svcA.Process", "Function");
    ctx_registry_add(reg, "Process", "proj.svcB.Process", "Function");

    ctx_resolution_t r = ctx_registry_resolve(reg, "Process", "proj.svcA.caller", NULL, NULL, 0);
...
TEST(registry_fuzzy_confidence_single) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Handler", "proj.svc.Handler", "Function");

    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknownPkg.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_fuzzy_confidence_distance) {
    ...
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknownPkg.Process", "proj.svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(registry_negative_import_rejects) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Process", "proj.billing.Process", "Function");
    ctx_registry_add(reg, "Process", "proj.handler.Process", "Function");

    /* Import only handler's module → should prefer handler */
    const char *keys[] = {"handler"};
    const char *vals[] = {"proj.handler"};
    ctx_resolution_t r = ctx_registry_resolve(reg, "Process", "proj.caller", keys, vals, 1);
...
TEST(registry_fuzzy_import_penalty) {
    ctx_registry_t *reg = ctx_registry_new();
    ctx_registry_add(reg, "Handler", "proj.billing.Handler", "Function");

    /* Has imports but billing not imported → confidence halved */
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknown.Handler", "proj.caller", keys, vals, 1);
    ASSERT_TRUE(fr.ok);
    /* 0.40 * 0.5 = 0.20 */
...
TEST(registry_fuzzy_no_import_map_passthrough) {
    ...
    ctx_fuzzy_result_t fr =
        ctx_registry_fuzzy_resolve(reg, "unknown.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT(fr.result.confidence > 0.35 && fr.result.confidence <= 0.45);
...
TEST(registry_find_by_name) {
    ...
    ctx_registry_add(reg, "Bar", "proj.pkg.Bar", "Function");
    ctx_registry_add(reg, "Foo", "proj.other.Foo", "Function");
    ctx_registry_add(reg, "transform", "proj.utils.DataProcessor.transform", "Method");

    /* FindByName returns all entries for "Foo" */
    const char **foos = NULL;
    int foos_count = 0;
    ctx_registry_find_by_name(reg, "Foo", &foos, &foos_count);
    ASSERT_EQ(foos_count, 2);

    /* FindByName for unique "Bar" */
    const char **bars = NULL;
    int bars_count = 0;
    ctx_registry_find_by_name(reg, "Bar", &bars, &bars_count);
    ...
    int trans_count = 0;
    ctx_registry_find_by_name(reg, "transform", &transforms, &trans_count);
    ASSERT_EQ(trans_count, 1);
    ASSERT_STR_EQ(transforms[0], "proj.utils.DataProcessor.transform");

    /* label_of */
    ASSERT_STR_EQ(ctx_registry_label_of(reg, "proj.utils.DataProcessor.transform"), "Method");
    ASSERT_STR_EQ(ctx_registry_label_of(reg, "proj.pkg.Foo"), "Function");

    /* Total size */
    ASSERT_EQ(ctx_registry_size(reg), 4);
    ...
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    r = ctx_registry_resolve(reg, "other.Foo", "proj.pkg", keys, vals, 1);
    ASSERT_STR_EQ(r.qualified_name, "proj.other.Foo");

    /* Resolve unique name */
    r = ctx_registry_resolve(reg, "Bar", "proj.unrelated", NULL, NULL, 0);
    ASSERT_STR_EQ(r.qualified_name, "proj.pkg.Bar");
...
TEST(infra_qn_helper) {
    ...
    char *qn = ctx_infra_qn("myproject", "docker-images/service/Dockerfile", "dockerfile", NULL);
    ASSERT_NOT_NULL(qn);
    ASSERT(strstr(qn, ".__infra__") != NULL);
    free(qn);

    /* Compose service → ::service_name suffix */
    qn = ctx_infra_qn("myproject", "docker-compose.yml", "compose-service", "web");
    ASSERT_NOT_NULL(qn);
...
TEST(infra_pipeline_integration) {
    ...
    /* Parse Dockerfile */
    ctx_dockerfile_result_t dr;
    ASSERT_EQ(ctx_parse_dockerfile_source("FROM alpine:3.19\nEXPOSE 8080\n", &dr), 0);
    ASSERT_STR_EQ(dr.base_image, "alpine:3.19");
    ASSERT_GTE(dr.port_count, 1);

    /* Parse .env */
    ctx_dotenv_result_t er;
    ASSERT_EQ(ctx_parse_dotenv_source("APP_PORT=8080\nDEBUG=true\n", &er), 0);
...
TEST(infra_pipeline_idempotent) {
    ...
    ctx_dockerfile_result_t r1, r2;
    ASSERT_EQ(ctx_parse_dockerfile_source(src, &r1), 0);
    ASSERT_EQ(ctx_parse_dockerfile_source(src, &r2), 0);
...
static const ctx_env_binding_t *find_binding_by_key(const ctx_env_binding_t *bindings, int count,
                                                    const char *key) {
    for (int i = 0; i < count; i++) {
...
static int has_binding_value(const ctx_env_binding_t *bindings, int count, const char *value) {
    for (int i = 0; i < count; i++) {
...
TEST(envscan_dockerfile_env_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_dock_XXXXXX");
    ...
    ctx_env_binding_t bindings[32];
    int count = ctx_scan_project_env_urls(tmpdir, bindings, 32);
...
TEST(envscan_shell_env_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_sh_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_env_file_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_env_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_toml_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_toml_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_yaml_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_yaml_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_terraform_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_tf_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_properties_urls) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_prop_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_secret_key_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_skey_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_secret_value_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_sval_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_secret_file_exclusion) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_sfile_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_skips_ignored_dirs) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_ign_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(envscan_non_url_values_skipped) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_envscan_nurl_XXXXXX");
    if (!ctx_mkdtemp(tmpdir))
        SKIP("tmpdir");
...
TEST(githistory_compute_change_coupling) {
    ...
    ctx_commit_files_t commits[5] = {
        {files_aaa, 3}, {files_bbb, 2}, {files_ccc, 2}, {files_ddd, 2}, {files_eee, 2},
    };

    ctx_change_coupling_t out[100];
    int count = ctx_compute_change_coupling(commits, 5, out, 100);
...
TEST(githistory_coupling_skips_large_commits) {
    ...
    ctx_commit_files_t commits[1] = {{files, 25}};

    ctx_change_coupling_t out[100];
    int count = ctx_compute_change_coupling(commits, 1, out, 100);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(githistory_coupling_limits_output) {
    ...
    ctx_commit_files_t commits[3675];
    int ci = 0;
    ...
    ctx_change_coupling_t out[200];
    int count = ctx_compute_change_coupling(commits, ci, out, 100);
    ASSERT(count <= 100);
    PASS();
}
...
TEST(registry_is_import_reachable) {
    ...
    ASSERT_TRUE(ctx_registry_is_import_reachable("proj.handler.Process", import_vals, 2));
    /* Sub-package: proj.handler.sub.Process → true (handler contains handler) */
    ASSERT_TRUE(ctx_registry_is_import_reachable("proj.handler.sub.Process", import_vals, 2));
    /* Nested match: proj.shared.utils.Helper → true */
    ASSERT_TRUE(ctx_registry_is_import_reachable("proj.shared.utils.Helper", import_vals, 2));
    /* Unrelated: proj.billing.Process → false */
    ASSERT_FALSE(ctx_registry_is_import_reachable("proj.billing.Process", import_vals, 2));
    /* Completely unrelated: unrelated.pkg.Func → false */
    ASSERT_FALSE(ctx_registry_is_import_reachable("unrelated.pkg.Func", import_vals, 2));
    PASS();
}
...
TEST(registry_find_ending_with) {
    ...
    int count = ctx_registry_find_ending_with(reg, "DataProcessor.transform", &matches);
    ASSERT_EQ(count, 1);
    ...
    matches = NULL;
    count = ctx_registry_find_ending_with(reg, "Foo", &matches);
    ASSERT_EQ(count, 2);
    ...
    matches = NULL;
    count = ctx_registry_find_ending_with(reg, "Nonexistent", &matches);
    ASSERT_EQ(count, 0);
...
static int setup_incremental_repo(void) {
    snprintf(g_incr_tmpdir, sizeof(g_incr_tmpdir), "/tmp/ctx_incr_XXXXXX");
    if (!ctx_mkdtemp(g_incr_tmpdir)) {
        return -1;
    }
...
TEST(pipeline_fastapi_depends_edges) {
    ...
    ctx_edge_t *edges = NULL;
    int edge_count = 0;
    ctx_store_find_edges_by_type(s, proj, "CALLS", &edges, &edge_count);
...
TEST(incremental_full_then_noop) {
    ...
    /* First: full index */
    ctx_pipeline_t *p = ctx_pipeline_new(g_incr_tmpdir, g_incr_dbpath, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(ctx_pipeline_run(p), 0);
    char *project = strdup(ctx_pipeline_project_name(p));
    ctx_pipeline_free(p);

    /* Verify nodes exist */
    ctx_store_t *s = ctx_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(s);
    int nodes_before = ctx_store_count_nodes(s, project);
    ASSERT_GT(nodes_before, 0);
    ctx_store_close(s);

    /* Second: incremental — nothing changed → should be no-op */
    p = ctx_pipeline_new(g_incr_tmpdir, g_incr_dbpath, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(ctx_pipeline_run(p), 0);
    ctx_pipeline_free(p);

    s = ctx_store_open_path(g_incr_dbpath);
    ASSERT_NOT_NULL(s);
    int nodes_after = ctx_store_count_nodes(s, project);
...
TEST(incremental_k8s_manifest_indexed) {
     ...
     * Verifies that ctx_pipeline_pass_k8s() runs during incremental re-index. */
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_k8s_incr_XXXXXX");
    if (!ctx_mkdtemp(tmpdir)) {
        SKIP("tmpdir");
    }
    char dbpath[512];
    ...
    /* Full index */
    ctx_pipeline_t *p = ctx_pipeline_new(tmpdir, dbpath, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(ctx_pipeline_run(p), 0);
    char *project = strdup(ctx_pipeline_project_name(p));
    ctx_pipeline_free(p);
    ...
    ctx_store_t *s = ctx_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    ctx_node_t *nodes = NULL;
    int count = 0;
    ctx_store_find_nodes_by_label(s, project, "Resource", &nodes, &count);
    ASSERT_GT(count, 0);
    ...
    p = ctx_pipeline_new(tmpdir, dbpath, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(ctx_pipeline_run(p), 0);
    ctx_pipeline_free(p);

    /* Verify both Resource nodes now present */
    s = ctx_store_open_path(dbpath);
    ASSERT_NOT_NULL(s);
    nodes = NULL;
    count = 0;
    ctx_store_find_nodes_by_label(s, project, "Resource", &nodes, &count);
...
TEST(incremental_kustomize_module_indexed) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_kust_incr_XXXXXX");
    if (!ctx_mkdtemp(tmpdir)) {
        SKIP("tmpdir");
    }
    char dbpath[512];
    ...
    ctx_node_t *nodes = NULL;
    int count = 0;
    ctx_store_find_nodes_by_label(s, project, "Module", &nodes, &count);
...
TEST(pipeline_lock_try_acquire) {
    ...
    ASSERT_TRUE(ctx_pipeline_try_lock());
    /* Second try-lock should fail (already held) */
    ASSERT_FALSE(ctx_pipeline_try_lock());
...
TEST(pipeline_lock_contention) {
    ...
    ctx_thread_t tid;
    int rc = ctx_thread_create(&tid, 0, try_lock_thread, NULL);
    ASSERT_EQ(rc, 0);
...
TEST(pipeline_empty_path) {
    ...
    ctx_pipeline_t *p = ctx_pipeline_new("", NULL, CTX_MODE_FULL);
...
TEST(pipeline_project_name_content) {
    ...
    ctx_pipeline_t *p = ctx_pipeline_new("/home/user/my-project", NULL, CTX_MODE_FULL);
    ASSERT_NOT_NULL(p);
    const char *name = ctx_pipeline_project_name(p);
    ASSERT_NOT_NULL(name);
    ASSERT_TRUE(strlen(name) > 0);
...
TEST(trackable_source_files) {
    ...
    ASSERT_TRUE(ctx_is_trackable_file("main.go"));
    ASSERT_TRUE(ctx_is_trackable_file("src/handler.py"));
    ASSERT_TRUE(ctx_is_trackable_file("lib/server.js"));
    ASSERT_TRUE(ctx_is_trackable_file("components/App.tsx"));
    ASSERT_TRUE(ctx_is_trackable_file("pkg/svc.rs"));
    ASSERT_TRUE(ctx_is_trackable_file("config.yaml"));
    PASS();
}

TEST(trackable_rejects_images) {
    /* Image and binary files are not trackable */
    ASSERT_FALSE(ctx_is_trackable_file("logo.png"));
    ASSERT_FALSE(ctx_is_trackable_file("photo.jpg"));
    ASSERT_FALSE(ctx_is_trackable_file("icon.gif"));
    ASSERT_FALSE(ctx_is_trackable_file("diagram.svg"));
    PASS();
}

TEST(trackable_rejects_minified) {
    /* Minified files are not trackable */
    ASSERT_FALSE(ctx_is_trackable_file("bundle.min.js"));
    ASSERT_FALSE(ctx_is_trackable_file("style.min.css"));
    PASS();
}

TEST(trackable_rejects_vendor_dirs) {
    /* Files in vendor/node_modules/__pycache__ are not trackable */
    ASSERT_FALSE(ctx_is_trackable_file("vendor/github.com/lib/dep.go"));
    ASSERT_FALSE(ctx_is_trackable_file("node_modules/lodash/index.js"));
    ASSERT_FALSE(ctx_is_trackable_file("__pycache__/module.pyc"));
    ASSERT_FALSE(ctx_is_trackable_file(".git/objects/pack/data"));
    PASS();
}
...
TEST(trackable_rejects_lock_files) {
    ...
    ASSERT_FALSE(ctx_is_trackable_file("package-lock.json"));
    ASSERT_FALSE(ctx_is_trackable_file("go.sum"));
    ASSERT_FALSE(ctx_is_trackable_file("yarn.lock"));
    PASS();
}

TEST(test_path_directory_patterns) {
    /* Test directory patterns: __tests__, test/, tests/, spec/ */
    ASSERT_TRUE(ctx_is_test_path("__tests__/Component.js"));
    ASSERT_TRUE(ctx_is_test_path("tests/test_main.py"));
    ASSERT_TRUE(ctx_is_test_path("spec/handler_spec.rb"));
    PASS();
}
...
TEST(test_path_suffix_patterns) {
    ...
    ASSERT_TRUE(ctx_is_test_path("handler.spec.ts"));
    ASSERT_TRUE(ctx_is_test_path("handler.test.tsx"));
    ASSERT_TRUE(ctx_is_test_path("handler_test.go"));
    ASSERT_TRUE(ctx_is_test_path("test_handler.py"));
    /* Non-test files */
    ASSERT_FALSE(ctx_is_test_path("handler.ts"));
    ASSERT_FALSE(ctx_is_test_path("contest.go"));
    ASSERT_FALSE(ctx_is_test_path("latest.py"));
    PASS();
}

TEST(test_func_name_go_patterns) {
    /* Go test function names: Test + uppercase char */
    ASSERT_TRUE(ctx_is_test_func_name("TestFoo"));
    ASSERT_TRUE(ctx_is_test_func_name("TestHTTPHandler"));
    /* Non-test: "Test" alone or Test + lowercase */
    ASSERT_FALSE(ctx_is_test_func_name("Testable")); /* lowercase 'a' after Test */
    PASS();
}

TEST(test_func_name_js_helpers) {
    /* JS/TS test helper function names */
    ASSERT_TRUE(ctx_is_test_func_name("it"));
    ASSERT_TRUE(ctx_is_test_func_name("describe"));
    ASSERT_TRUE(ctx_is_test_func_name("test"));
    ASSERT_TRUE(ctx_is_test_func_name("beforeEach"));
    ASSERT_TRUE(ctx_is_test_func_name("afterEach"));
    PASS();
}

TEST(env_var_name_valid) {
    /* Valid env var names: uppercase + underscores, at least 2 chars with uppercase */
    ASSERT_TRUE(ctx_is_env_var_name("DATABASE_URL"));
    ASSERT_TRUE(ctx_is_env_var_name("API_KEY"));
    ASSERT_TRUE(ctx_is_env_var_name("PORT"));
    ASSERT_TRUE(ctx_is_env_var_name("DB_2"));
    ASSERT_TRUE(ctx_is_env_var_name("MY_VAR_123"));
    PASS();
}

TEST(env_var_name_invalid) {
    /* Invalid: too short, lowercase, mixed case, empty */
    ASSERT_FALSE(ctx_is_env_var_name("A"));      /* single char */
    ASSERT_FALSE(ctx_is_env_var_name("port"));   /* lowercase */
    ASSERT_FALSE(ctx_is_env_var_name("apiKey")); /* camelCase */
    ASSERT_FALSE(ctx_is_env_var_name("__"));     /* no uppercase */
    ASSERT_FALSE(ctx_is_env_var_name(""));       /* empty */
    ASSERT_FALSE(ctx_is_env_var_name("123"));    /* digits only */
    PASS();
}

TEST(config_ext_positive) {
    /* Config file extensions recognized */
    ASSERT_TRUE(ctx_has_config_extension(".env"));
    ASSERT_TRUE(ctx_has_config_extension("config.yaml"));
    ASSERT_TRUE(ctx_has_config_extension("config.yml"));
    ASSERT_TRUE(ctx_has_config_extension("settings.toml"));
    ASSERT_TRUE(ctx_has_config_extension("app.ini"));
    ASSERT_TRUE(ctx_has_config_extension("data.json"));
    ASSERT_TRUE(ctx_has_config_extension("app.properties"));
    PASS();
}

TEST(config_ext_negative) {
    /* Non-config extensions rejected */
    ASSERT_FALSE(ctx_has_config_extension("main.go"));
    ASSERT_FALSE(ctx_has_config_extension("app.py"));
    ASSERT_FALSE(ctx_has_config_extension("handler.rs"));
    ASSERT_FALSE(ctx_has_config_extension("data.csv"));
    ASSERT_FALSE(ctx_has_config_extension("README.md"));
    PASS();
}
...
TEST(split_camel_basic) {
    ...
    n = ctx_split_camel_case("getCamelCase", parts, 16);
    ASSERT_EQ(n, 3);
    ASSERT_STR_EQ(parts[0], "get");
...
TEST(split_camel_single_word) {
    ...
    char *parts[16];
    int n = ctx_split_camel_case("hello", parts, 16);
    ASSERT_EQ(n, 1);
...
TEST(split_camel_empty) {
    ...
    char *parts[16];
    int n = ctx_split_camel_case("", parts, 16);
    for (int i = 0; i < n; i++)
        free(parts[i]);
...
TEST(tokenize_decorator_login_required) {
    ...
    char *tokens[16];
    int n = ctx_tokenize_decorator("@login_required", tokens, 16);
    ASSERT_EQ(n, 2);
    ASSERT_STR_EQ(tokens[0], "login");
    ASSERT_STR_EQ(tokens[1], "required");
    for (int i = 0; i < n; i++)
...
TEST(tokenize_decorator_single) {
    /* @Override → ["override"] */
    char *tokens[16];
    int n = ctx_tokenize_decorator("@Override", tokens, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(tokens[0], "override");
    for (int i = 0; i < n; i++)
        free(tokens[i]);
...
TEST(split_command_basic) {
    ...
    char *args[16];
    int n = ctx_split_command("gcc -c main.c -o main.o", args, 16);
    ASSERT_EQ(n, 5);
    ASSERT_STR_EQ(args[0], "gcc");
    ASSERT_STR_EQ(args[1], "-c");
    ASSERT_STR_EQ(args[2], "main.c");
...
TEST(split_command_quoted) {
    ...
    char *args[16];
    int n = ctx_split_command("gcc -DFOO=\"bar baz\" main.c", args, 16);
    ASSERT_GTE(n, 3);
    ASSERT_STR_EQ(args[0], "gcc");
    for (int i = 0; i < n; i++)
        free(args[i]);
...
TEST(split_command_empty) {
    ...
    char *args[16];
    int n = ctx_split_command("", args, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(parse_range_single) {
    /* "42" → start=42, count=1 */
    int start, count;
    ctx_parse_range("42", &start, &count);
    ASSERT_EQ(start, 42);
    ASSERT_EQ(count, 1);
    PASS();
}
...
TEST(parse_range_zero_count) {
    ...
    int start, count;
    ctx_parse_range("100,0", &start, &count);
    ASSERT_EQ(start, 100);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(parse_name_status_basic) {
    ...
    ctx_changed_file_t files[16];
    int n = ctx_parse_name_status(input, files, 16);
    /* Exact count depends on which files pass ctx_is_trackable_file */
    ASSERT_GTE(n, 0);
    ASSERT_LTE(n, 3);
    PASS();
}
...
TEST(parse_name_status_empty) {
    /* Empty input */
    ctx_changed_file_t files[16];
    int n = ctx_parse_name_status("", files, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(parse_hunks_empty) {
    /* Empty input */
    ctx_changed_hunk_t hunks[16];
    int n = ctx_parse_hunks("", hunks, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(coupling_empty_commits) {
    ...
    ctx_change_coupling_t results[16];
    int n = ctx_compute_change_coupling(NULL, 0, results, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(coupling_single_file_commit) {
    ...
    char *f1[] = {"a.go"};
    char *f2[] = {"b.go"};
    ctx_commit_files_t commits[] = {{f1, 1}, {f2, 1}};
    ctx_change_coupling_t results[16];
    int n = ctx_compute_change_coupling(commits, 2, results, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(clean_json_brackets_array) {
    ...
    char out[256];
    ctx_clean_json_brackets("[\"./app\", \"--flag\"]", out, sizeof(out));
...
TEST(clean_json_brackets_empty) {
    ...
    char out[256];
    ctx_clean_json_brackets("", out, sizeof(out));
    ASSERT_STR_EQ(out, "");
    PASS();
}
...
TEST(fqn_compute_basic) {
    ...
    char *fqn = ctx_pipeline_fqn_compute("proj", "pkg/handler.go", "Serve");
    ASSERT_NOT_NULL(fqn);
...
TEST(fqn_compute_strips_ext) {
    ...
    char *fqn = ctx_pipeline_fqn_compute("proj", "main.go", "main");
    ASSERT_NOT_NULL(fqn);
...
TEST(fqn_module_basic) {
    ...
    char *mod = ctx_pipeline_fqn_module("proj", "pkg/util/helper.go");
    ASSERT_NOT_NULL(mod);
...
TEST(fqn_folder_basic) {
    ...
    char *folder = ctx_pipeline_fqn_folder("proj", "pkg/util");
...
TEST(project_name_trailing_slash) {
    ...
    char *a = ctx_project_name_from_path("/home/user/project");
    ASSERT_NOT_NULL(a);
    free(a);
    PASS();
}
