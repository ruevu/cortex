...
static int run_configlink(ctx_gbuf_t *gb, const char *project, const char *repo_path) {
    ...
    return ctx_pipeline_pass_configlink(&ctx);
}
#include <sys/stat.h>
#include <unistd.h>
...
static bool has_strategy(const ctx_gbuf_edge_t **edges, int count, const char *strategy) {
    char needle[64];
...
static bool has_strategy_with_confidence(const ctx_gbuf_edge_t **edges, int count,
...
static bool has_strategy_with_key(const ctx_gbuf_edge_t **edges, int count, const char *strategy,
...
TEST(configlink_key_symbol_exact_match) {
    ctx_gbuf_t *gb = ctx_gbuf_new("test", "/tmp/test");

    /* Config Variable node: max_connections from config.toml */
    int64_t cfg_id = ctx_gbuf_upsert_node(gb, "Variable", "max_connections",
                                          "test.config.max_connections", "config.toml", 0, 0, NULL);
    ASSERT_GT(cfg_id, 0);

    /* Code Function node: getMaxConnections from main.go */
    int64_t func_id = ctx_gbuf_upsert_node(gb, "Function", "getMaxConnections",
    ...
    int count = 0;
    ctx_gbuf_find_edges_by_type(gb, "CONFIGURES", &edges, &count);
...
TEST(configlink_key_symbol_substring_match) {
    ...
    ctx_gbuf_upsert_node(gb, "Variable", "request_timeout", "test.config.request_timeout",
                         "config.toml", 0, 0, NULL);

    ctx_gbuf_upsert_node(gb, "Function", "getRequestTimeoutSeconds",
...
TEST(configlink_key_symbol_short_key_skipped) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        ctx_gbuf_upsert_node(gb, "Variable", short_keys[i], qn, "config.toml", 0, 0, NULL);
    }

    ctx_gbuf_upsert_node(gb, "Function", "getPort", "test.main.getPort", "main.go", 0, 0, NULL);
...
TEST(configlink_key_symbol_camel_case) {
    ...
    ctx_gbuf_upsert_node(gb, "Variable", "maxRetryCount", "test.config.maxRetryCount",
                         "config.json", 0, 0, NULL);

    ctx_gbuf_upsert_node(gb, "Function", "getMaxRetryCount", "test.handler.getMaxRetryCount",
                         "handler.go", 0, 0, NULL);
...
TEST(configlink_key_symbol_no_false_positive) {
    ...
    ctx_gbuf_upsert_node(gb, "Variable", "url", "test.db.url", "config.toml", 0, 0, NULL);

    ctx_gbuf_upsert_node(gb, "Function", "parseURL", "test.main.parseURL", "main.go", 0, 0, NULL);
...
TEST(configlink_dep_import_package_json) {
    ...
    ctx_gbuf_upsert_node(gb, "Variable", "express", "test.package.dependencies.express",
                         "package.json", 0, 0, NULL);

    /* Module node for app.js (source of import) */
    int64_t app_id = ctx_gbuf_upsert_node(gb, "Module", "app", "test.app", "app.js", 0, 0, NULL);

    /* Import target node (what 'express' resolves to) */
    int64_t target_id = ctx_gbuf_upsert_node(gb, "Module", "express", "test.node_modules.express",
                                             "node_modules/express/index.js", 0, 0, NULL);

    /* IMPORTS edge: app.js → express module */
    ctx_gbuf_insert_edge(gb, app_id, target_id, "IMPORTS", NULL);
...
TEST(configlink_file_ref_no_false_positive) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ctx_cfglink_XXXXXX");
    ASSERT_NOT_NULL(ctx_mkdtemp(tmpdir));
    ...
    char *project = ctx_project_name_from_path(tmpdir);
    ctx_gbuf_t *gb = ctx_gbuf_new(project, tmpdir);

    /* data.csv Module (NOT a config extension) */
    char *csv_qn = ctx_pipeline_fqn_module(project, "data.csv");
    ctx_gbuf_upsert_node(gb, "Module", "data", csv_qn, "data.csv", 0, 0, NULL);

    /* Source Module */
    char *main_qn = ctx_pipeline_fqn_module(project, "main.go");
    ctx_gbuf_upsert_node(gb, "Module", "main", main_qn, "main.go", 0, 0, NULL);
