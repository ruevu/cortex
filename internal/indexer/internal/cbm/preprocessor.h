#ifndef CTX_PREPROCESSOR_H
#define CTX_PREPROCESSOR_H
...
// The returned string must be freed with ctx_preprocess_free().
char *ctx_preprocess(const char *source, int source_len, const char *filename,
                     const char **extra_defines, const char **include_paths, int cpp_mode);

// Free preprocessed source returned by ctx_preprocess.
